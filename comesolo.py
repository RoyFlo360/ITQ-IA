...
class Pegboard:
    ...
    # Returns True if the board is solved, i.e. only 1 peg remains.
    def is_solved(self):
    ...
    def move_peg(self, from_peg, to_peg, over_peg):
...
def solve_pegboard(pegboard, moves_so_far=None):
    ...
    while True:
        ...
        while legal_moves:
            ...
            new_pegboard = pegboard.move_peg(from_peg, to_peg, over_peg)

            # Check to see if we've solved it.
            if new_pegboard.is_solved():
            ...
            # Recurse.
            solve_pegboard(new_pegboard, new_moves)
...
while True:
    missing = input("Welcome to the Peg Solitaire Solver. Start the puzzle with which peg missing? (1-15))\n")
    try:
        missing = int(missing)
